...
//				 )
	function CDBTree(&$DB, $tableName, $itemId, $fieldNames=array()) {
		if((empty($tableName)) || (empty($itemId))) die("phpDbTree error: ".$this->db->error());
		$this->db = $DB;
...
//************************************************************************
// Returns a Left and Right IDs, Level and Parent of an element or false on error
// $ID : an ID of the element
	function getElementInfo($ID) { return $this->getNodeInfo($ID); }
	function getNodeInfo($ID) {
		$this->sql = 'SELECT '.$this->left.','.$this->right.','.$this->level.','.$this->parent.' FROM '.$this->table.' WHERE '.$this->id.'=\''.$ID.'\'';
		if(($query=$this->db->query($this->sql)) && ($this->db->num_rows($query) == 1) && ($Data = $this->db->fetch_array($query)))
			return array((int)$Data[$this->left], (int)$Data[$this->right], (int)$Data[$this->level], (int)$Data[$this->parent]); 
...
// Returns : true on success, or false on error
	function insert($ID, $data) {
		...
		$fld_values .= $ID.','.($rightId).','.($rightId+1).','.($level+1);
...
// Returns : false on error 
    function moveAll($ID, $newParentId) { 
        if(!(list($leftId, $rightId, $level, $oldParentId) = $this->getNodeInfo($ID))) die("phpDbTree error: ".$this->db->error()); 
        ...
        $this->sql = 'UPDATE '.$this->table.' SET '.$this->parent.'='.$newParentId.' WHERE '.$this->id.'='.$ID;
...
// Returns : true on success, or false on error
	function delete($ID) {
        ...
        $this->sql = 'SELECT '.$this->parent.' FROM '.$this->table.' WHERE '.$this->id.'='.$ID;
        if(!$query = $this->db->query($this->sql)) die($this->db->error());
        $data = $this->db->fetch_array($query);
        $parentId = $data[$this->parent];
		// Deleting record
		$this->sql = 'DELETE FROM '.$this->table.' WHERE '.$this->id.'=\''.$ID.'\'';
		if(!$this->db->query($this->sql)) die("phpDbTree error: ".$this->db->error());

		// Clearing blank spaces in a tree and updating parent
		$this->sql = 'UPDATE '.$this->table.' SET '
			...
			. $this->left.'=IF('.$this->left.'>'.$rightId.','.$this->left.'-2,'.$this->left.'),'
			. $this->right.'=IF('.$this->right.'>'.$rightId.','.$this->right.'-2,'.$this->right.'),'
			. $this->parent.'=IF('.$this->parent.'='.$ID.','.$parentId.','.$this->parent.') '
...
// Returns : a result id for using with other DB functions
	...
	function enumChildren($ID, $start_level=1, $end_level=1) {
		if($start_level < 0) die("phpDbTree error: ".$this->db->error());
