...
<td style='padding-right: 2px;'>
<input class='{if $tab==1}tabOn{else}tabOff{/if}' id='tab1' type='button' value='{if $type == "fact" || $type == "formula"}Формулировки из учебников{else}Определения из учебников{/if}' onclick='showTabPage(1,6);document.term.tab.value=1;'>
</td>
<td style='padding-right: 2px;'>
<input class='{if $tab==2}tabOn{else}tabOff{/if}' id='tab2' type='button' value='{if $type == "fact" || $type == "formula"}Сравнение формулировок{else}Сравнение определений{/if}' onclick='showTabPage(2,6);document.term.tab.value=2;'>
</td>
<td style='padding-right: 2px;'>
...
<tr><td class=tblheader1 width="250">Является ссылкой на статью:</td><td>
<input type="hidden" name="ref" value="{$ref}">
<nobr>
<input type=text size="40" name="ref_title" value="{$ref_title}"/>
<a href="#" title="Выбрать статью" onclick="popup=window.open('/admin/dic/term_picker.php?form_name=term&element_name=ref&text_element_name=ref_title','_picker','width=620,height=420,left=' + ((screen.width-620)/2) + ',top=' + ((screen.height-420)/2) + ',scrollbars=yes,resizable'); popup.focus(); return false;">
...
{if $type == "fact"}
<tr><td class=tblheader1 valign=top>Вид факта</td><td>
<select name="fact_type">{html_options options=$fact_type_options selected=$fact_type}</select>
</td></tr>
{/if}
...
<tr><td class=tblheader1 valign=top>Код источника</td><td>
введите
<input type="text" size="32" name="src_comment_code" value="{$src_options[$src_comment]}">
или выберите
<select name="src_comment"><option value="0" label="---">---</option>{html_options options=$src_options selected=$src_comment}</select>
</td></tr>
<tr><td class=tblheader1 valign=top>Иллюстративный материал</td><td>
<textarea cols=60 rows=10 name="illustration">{$illustration}</textarea>
</td></tr>
<tr><td class=tblheader1 valign=top>Код источника</td><td>
введите
<input type="text" size="32" name="src_illustration_code" value="{$src_options[$src_illustration]}">
или выберите
<select name="src_illustration"><option value="0" label="---">---</option>{html_options options=$src_options selected=$src_illustration}</select>
</td></tr>
<tr><td class=tblheader1 valign=top>Историческая справка</td><td>
<textarea cols=60 rows=10 name="history">{$history}</textarea>
</td></tr>
<tr><td class=tblheader1 valign=top>Код источника</td><td>
введите
<input type="text" size="32" name="src_history_code" value="{$src_options[$src_history]}">
или выберите
<select name="src_history"><option value="0" label="---">---</option>{html_options options=$src_options selected=$src_history}</select>
</td></tr>
</table>
...
{include file="generic_list.tpl" _rows=$def _columns=$def_columns _show_insert="1" _href="/admin/dic/wording.php?term=$id&" _related="wording" _picker_href="/admin/dic/wording.php?term=$id&" _delete_column="1" _delete_message="Удалить определение?" _delete_href="/admin/dic/term.php?id=$id&tab=1&del_wording=" _no_form="1"}

</span>
<span style='background-color:#F2F2F2;display:{if $tab==2}block{else}none{/if};' id='page2'>

{include file="generic_list.tpl" _rows=$comp _columns=$comp_columns _show_insert="1" _href="/admin/dic/comp.php?term=$id&" _related="comp" _picker_href="/admin/dic/comp.php?term=$id&" _delete_column="1" _delete_message="Удалить сравнение?" _delete_href="/admin/dic/term.php?id=$id&tab=2&del_comp=" _no_form="1"}
...
<table width=100%>
{foreach from=$change_history item=h key=i}
<tr>
